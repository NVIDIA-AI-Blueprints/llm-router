...
MODEL_ROUTER_TO_TARGET = {
    ...
    # Qwen VL models -> Qwen/Qwen3-VL-8B-Instruct
...
CUSTOM_THRESHOLDS = {
    'gpt-5-chat': 0.70,
    'Qwen/Qwen3-VL-8B-Instruct': 0.55,
...
def map_router_model_to_target(router_model: str) -> str:
    ...
    elif 'qwen' in lower_model or 'vl' in lower_model:
        logger.warning(f"Router model '{router_model}' not in mapping, assuming Qwen VL model")
...
async def nn_objective_fn(config: NNObjectiveConfig, _builder: Builder):
    ...
    - HuggingFace embedding model (Qwen VL) for multimodal content encoding
